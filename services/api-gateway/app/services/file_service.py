...
class FileService:
    ...
    def validate_homework_file(self, file: UploadFile) -> Dict:
        ...
        # Get file size (simplified - in production, read file to get size)
        file_size = 0
        
        if file.content_type not in allowed_types:
            return {
                "valid": False,
                "error": f"File type {file.content_type} not allowed. Allowed types: {', '.join(allowed_types)}"
            }
    ...
    async def upload_file(
        ...
        logger.info(f"Uploaded file {filename} to {file_url}")
