...
class Learner(BaseModel):
    ...
    iep = relationship("IEP", back_populates="learner", uselist=False, cascade="all, delete-orphan")
    homework_sessions = relationship(
        "HomeworkSession", back_populates="learner", cascade="all, delete-orphan"
    )
    iep_goals = relationship("IEPGoal", back_populates="learner", cascade="all, delete-orphan")
    regulation_sessions = relationship(
        "RegulationSession", back_populates="learner", cascade="all, delete-orphan"
    )
    emotion_history = relationship(
        "EmotionHistory", back_populates="learner", cascade="all, delete-orphan"
    )
    progress_records = relationship(
        "ProgressRecord", back_populates="learner", cascade="all, delete-orphan"
    )
