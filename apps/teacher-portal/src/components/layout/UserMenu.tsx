    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center text-white font-semibold">
          {user?.name ? user.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'MS'}
        </div>
        <div className="hidden md:block text-right">
          <p className="text-sm font-semibold text-neutral-900">{user?.name || 'Ms. Smith'}</p>
          <p className="text-xs text-neutral-500">Special Ed Teacher</p>
        </div>
        <svg
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-neutral-200 py-2 z-50">
          ...
          <div className="md:hidden px-4 py-3 border-b border-neutral-200">
            <p className="text-sm font-medium text-neutral-900">{user?.name || 'Ms. Smith'}</p>
            <p className="text-xs text-neutral-500">{user?.email || 'teacher@school.edu'}</p>
          </div>
